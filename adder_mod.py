...
class State(object):
    ...
    def save_cell_map(self):
        ...
        if os.path.exists("cell_map/adder_{}b/adder_{}b_{}l_{}s_{}.log".format(self.input_bit,
                                                                               self.input_bit, int(self.level),
                                                                               int(self.size),
                                                                               int(args.max_save) - 1)):
            return
        i = 0
        while os.path.exists(
                "cell_map/adder_{}b/adder_{}b_{}l_{}s_{}.log".format(self.input_bit,
                                                                     self.input_bit, int(self.level),
                                                                     int(self.size), i)):
            i += 1
        fdot_save = open(
    ...
    def compute_reward(self):
        size_weight = 1.0
        level_weight = 1.0
        return - (size_weight * self.size + level_weight * self.level)
    ...
    def get_next_state_with_random_choice(self, set_action=None, remove_action=None):
        ...
        for choice, _ in heuristic_scores:
            ...
            next_level_map = self.update_level_map(next_cell_map, next_level_map, start_bit=x,
                                                   activate_x_list=activate_x_list)
...
def default_policy(node):
    ...
    while current_state.is_terminal() == False:
        ...
        if min_size > current_state.size:
            best_cell_map = current_state.cell_map
            time_log.write("{}\t{}\t{:.2f}\t{}\n".format(current_state.level,
                                                         current_state.size, time.time() - global_start_time,
                                                         global_step))
            time_log.flush()
        min_size = min(min_size, current_state.size)
        current_state.save_cell_map()
...
def monte_carlo_tree_search(node, computation_budget):
    ...
    iterations = computation_budget // simulations_per_iteration
