...
class State(object):
    ...
    def save_cell_map(self):
        ...
        fdot_save = open("cell_map_{}b_{}_{}/cell_map_{}b_{}_{}_{}.log".format(self.input_bit, int(self.level),
                                                                               int(self.size), self.input_bit,
                                                                               int(self.level), int(self.size), strftime), 'w')
    ...
    def run_openroad(self):
        ...
        verilog_file_path = "mult_adder_tmp_{}.v".format(file_name_prefix)

        if self.error == False:
            yosys_file_name = os.path.join("run_yosys_mult_add_mid",
                                           self.verilog_file_name.split(".")[0] + "_yosys.v")
            shutil.copyfile(yosys_file_name, verilog_file_path)
        fopen_tcl = open("nangate45_mult_adder_{}.tcl".format(file_name_prefix), "w")
        fopen_tcl.write(openroad_tcl.format("mult_adder_tmp_{}.v".format(file_name_prefix)))
        fopen_tcl.close()

        output = subprocess.check_output(['openroad',
                                          "nangate45_mult_adder_{}.tcl".format(file_name_prefix)],
        ...
        verilog_file_path = "mult_tmp_{}_wo_flatten.v".format(file_name_prefix)

        if self.wo_error == False:
            yosys_file_name = os.path.join("run_yosys_mult_mid",
                                           self.verilog_file_name.split(".")[0] + "_wo_flatten_yosys.v")
            shutil.copyfile(yosys_file_name, verilog_file_path)

            fopen_tcl = open("nangate45_mult_{}.tcl".format(file_name_prefix), "w")
            ...
            output_wo_flatten = subprocess.check_output(['openroad',
                                                         "nangate45_mult_{}.tcl".format(file_name_prefix)],
                                                        shell=True).decode('utf-8')
        ...
        os.remove("nangate45_mult_adder_{}.tcl".format(file_name_prefix))
        delay *= 1000
        delay_wo *= 1000
        self.delay = delay
        self.area = area
    ...
    def get_next_state_with_heuristic_choice(self):
        global global_step, global_iter
        if self.available_choice == 0:
            return None

        best_action = None
        min_level_increase = float('inf')

        for action in self.available_choice_list:
            ...
            if action_type == 0:
                ...
                next_cell_map, next_min_map = self.legalize(next_cell_map, next_min_map)
            next_level_map = self.update_level_map(next_cell_map, next_level_map)
            level_increase = next_level_map.max() - self.level

            if level_increase < min_level_increase:
                min_level_increase = level_increase
                best_action = action
...
def default_policy(node, level_bound_delta):
    ...
    step = 0
    max_steps = INPUT_BIT // 16
    while not current_state.is_terminal() and step < max_steps:
        current_state = current_state.get_next_state_with_heuristic_choice()
        if global_iter >= args.max_iter:
            return None
