...
def best_child(node, is_exploration):
    ...
    best_sub_node = None
    C = 1 / math.sqrt(2.0) if is_exploration else 0.0
    for sub_node in node.get_children():
        if sub_node.get_visit_times() == 0:
            ...
            heuristic = sub_node.get_state().compute_reward()
            score = exploitation + C * exploration + heuristic * 0.1  # 调整权重
        if score > best_score:
