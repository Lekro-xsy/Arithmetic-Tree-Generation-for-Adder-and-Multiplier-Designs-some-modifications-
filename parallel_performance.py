...
class State(object):
    ...
    def save_cell_map(self):
        ...
        if os.path.exists("cell_map/adder_{}b/adder_{}b_{}l_{}s_{}.log".format(self.input_bit,
            ...
            return
        i = 0
        file_path = "cell_map/adder_{}b/adder_{}b_{}l_{}s_{}.log".format(self.input_bit, 
            self.input_bit, int(self.level), int(self.size), i)
        while os.path.exists(file_path):
            i += 1
            file_path = "cell_map/adder_{}b/adder_{}b_{}l_{}s_{}.log".format(self.input_bit, 
                self.input_bit, int(self.level), int(self.size), i)
        fdot_save = open(file_path, 'w')
        for ii in range(self.input_bit):
            for jj in range(self.input_bit):
                fdot_save.write("{} ".format(str(int(self.cell_map[ii, jj]))))
            fdot_save.write("\n")
        fdot_save.write("\n")
        fdot_save.close()
...
def get_sklansky_init():
    ...
    state = State(level, size, cell_map, level_map, min_map,
            0, 0, 0)
    state.available_choice = int(state.prob[1].sum())
    state.available_choice_list = []
    for i in range(INPUT_BIT):
        for j in range(INPUT_BIT):
            if state.prob[1,i,j] == 1:
                state.available_choice_list.append(INPUT_BIT **2 + i* INPUT_BIT+j)
...
def default_policy_wrapper(node):
    import copy
    node_copy = copy.deepcopy(node)
    reward = default_policy(node_copy)
    return reward, node_copy

def parallel_simulation(nodes):
    with multiprocessing.Pool() as pool:
        results = pool.map(default_policy_wrapper, nodes)
    return results

def monte_carlo_tree_search_parallel(node, computation_budget):
    global update
    global best_cell_map
    global min_size
    global INPUT_BIT
    ...
    start_time = time.time()
    simulations_per_iteration = multiprocessing.cpu_count()
    iterations = max(1, computation_budget // simulations_per_iteration)
    for i in range(iterations):
        print("Iteration {}/{}".format(i+1, iterations))

        nodes_to_simulate = []
        for _ in range(simulations_per_iteration):
            selected_node = tree_policy(node)
            nodes_to_simulate.append(selected_node)

        simulation_results = parallel_simulation(nodes_to_simulate)
        for result in simulation_results:
            reward, simulated_node = result
            backup(simulated_node, reward)

        assert node.parent is None
        print("Current best size: {}, level: {}".format(
            min_size, int(math.log2(INPUT_BIT) + 1 + LEVEL_BOUND_DELTA)))

    print("Search completed.")
    best_next_node = best_child(node, False)
    end_time = time.time()
    return best_next_node, end_time - start_time

def run_performance_test(node, computation_budget=1000):
    start_time = time.time()
    best_node_original = monte_carlo_tree_search(copy.deepcopy(node), computation_budget)
    time_original = time.time() - start_time
    global best_result, min_size, best_cell_map, update, global_step, global_start_time
    best_result = {}
    min_size = node.get_state().size
    best_cell_map = node.get_state().cell_map
    update = False
    global_step = 0
    global_start_time = time.time()
    start_time = time.time()
    best_node_parallel, time_parallel = monte_carlo_tree_search_parallel(copy.deepcopy(node), computation_budget)
    time_parallel_total = time.time() - start_time

    print("Performance Comparison:")
    print(f"Original MCTS Time: {time_original:.4f}s")
    print(f"Parallel MCTS Time: {time_parallel_total:.4f}s")
    print("Original Best Node State Size:", best_node_original.get_state().size if best_node_original else None)
    print("Parallel Best Node State Size:", best_node_parallel.get_state().size if best_node_parallel else None)


def main():
    init_state = get_sklansky_init()
    global min_size, best_result, best_cell_map, update, global_step, global_start_time
    min_size = init_state.size
    best_result = {}
    best_cell_map = init_state.cell_map
    update = False
    ...
    init_node = Node()
    init_node.set_state(init_state)
    run_performance_test(init_node, computation_budget=50) 
